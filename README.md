...
- 베들레헴 폴더 일괄 가져오기와 `.bdb`, `.sdb`, `.cdb`, `.dct`, `.hdb`, `.cmp` 직접 가져오기
...
- 찬송가 번호·제목·가사 검색과 `.cmp` 악보 보기
- 데스크톱 3단 연구 화면과 모바일 탭 화면
- 오프라인 캐시와 홈 화면 설치를 지원하는 PWA

## 베들레헴 자료 연결

앱은 한글·영문 공개 역본 4종과 히브리어·헬라어 원문 분해를 별도 작업 없이 제공합니다. 추가로 보유한 자료가 있다면 `내 서재` → `베들레헴 폴더 전체 가져오기`를 누르고 기존 베들레헴 자료 폴더를 한 번 선택합니다. 지원 파일만 순서대로 저장하며 DLL, 실행 파일과 목록 문서는 자동으로 제외합니다. 필요한 자료만 넣으려면 `필요한 파일만 선택`을 사용합니다. 추천 순서는 다음과 같습니다.
...
6. `새찬송가.cmp` — 찬송가 악보 이미지

가져온 원본 파일은 브라우저 IndexedDB에만 저장되며 서버로 업로드되지 않습니다. 같은 이름의 파일을 다시 가져오면 중복 저장하지 않고 기존 자료를 갱신합니다. 전체 자료 폴더는 1GB를 넘을 수 있으므로 기기 저장 공간을 충분히 확보해야 합니다. 단, 사용자가 `이 절의 통찰 보기`를 누르면 선택한 절, 앞뒤 절과 원어 분석 정보가 Cloudflare Workers AI로 전송됩니다. 생성 결과는 연구 보조 자료이며 설교자의 문맥·신학 검토를 대신하지 않습니다.
